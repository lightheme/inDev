...
const run = async () => {
  ...
  const tick = async () => {
    ...
    running = true;
    try {
      await queue.tickDelayed(queueName, config.queue.tickBatchSize);
      await queue.recoverInflight(queueName, config.queue.inflightTimeoutMs, config.queue.recoverBatchSize);
