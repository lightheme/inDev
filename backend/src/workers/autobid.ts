  ...
  const tick = async () => {
    ...
    try {
      ...
      for (const auction of auctions) {
        const roundIndex = getActiveRoundNumber(auction);
        if (roundIndex === null) continue;

        const round = auction.rounds[roundIndex];
        const bidRoundNumber = round.roundNumber;
        ...
        eligibleRounds.push({ auctionId: auction._id.toString(), roundNumber: roundIndex });
      }
